...
def parse_llm_response(response):
    try:
        ...
        # Split by likely source section
            ...
            for line in source_lines:
                ...
                if url_match:
                    ...
                    if text_match:
                        ...
                        text = text_value.strip() if text_value else text

                    if url and url.startswith("http"):
                        links.append({"url": url, "text": text})

        logger.info(f"Parsed answer (length: {len(answer)}) and {len(links)} links")
        return {
            "answer": answer if answer else "I couldn't find any relevant information in my knowledge base.",
            "links": links
        }
...
async def query_knowledge_base(request: QueryRequest):
    try:
        ...
        try:
            ...
            result["answer"] = result.get("answer", "I couldn't find any relevant information in my knowledge base.")
            result["links"] = result.get("links", [])
